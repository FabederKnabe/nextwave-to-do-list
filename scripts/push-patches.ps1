...
<#
  ...
  Endpoint hardcoded. Service-Role-Key aus .env.local im Repo-Root (gitignored),
  nur fuer die Dauer des Laufs in der Prozessumgebung. -CheckEnv prueft nur das.
#>
...
# Nur .env.local zaehlt: eine geerbte Umgebungsvariable wird verworfen, sonst verdeckt
# ein fremder Wert still die Datei.
$EnvFile = Join-Path $PSScriptRoot '..\.env.local'
if (-not (Test-Path -LiteralPath $EnvFile)) { throw "Abbruch: $EnvFile fehlt (SUPABASE_SERVICE_ROLE_KEY)." }
$env:SUPABASE_SERVICE_ROLE_KEY = $null
foreach ($line in Get-Content -LiteralPath $EnvFile) {
  if ($line -match '^\s*SUPABASE_SERVICE_ROLE_KEY\s*=\s*(\S+)\s*$') { $env:SUPABASE_SERVICE_ROLE_KEY = $matches[1] }
}
$ServiceKey = $env:SUPABASE_SERVICE_ROLE_KEY
if (-not $ServiceKey) { throw "Abbruch: SUPABASE_SERVICE_ROLE_KEY fehlt in $EnvFile." }
if ($CheckEnv) { Write-Output "Env ok: SUPABASE_SERVICE_ROLE_KEY aus $EnvFile geladen."; exit 0 }
